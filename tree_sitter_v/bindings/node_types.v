...
pub fn (typ NodeType) group() SuperType {
	return if typ in bindings.supertype__top_level_declaration_nodes {
		SuperType.top_level_declaration
	} else if typ in bindings.supertype__expression_nodes {
		SuperType.expression
	} else if typ in bindings.supertype__statement_nodes {
		SuperType.statement
	} else {
		SuperType.unknown
	}
...
pub fn (typ NodeType) is_declaration() bool {
	return typ in bindings.declaration_node_types
}

pub fn (typ NodeType) is_identifier() bool {
	return typ in bindings.identifier_node_types
}

pub fn (typ NodeType) is_literal() bool {
	return typ in bindings.literal_node_types
}
...
pub fn (nf VNodeTypeFactory) get_type(type_name string) NodeType {
	return bindings.node_type_name_to_enum[type_name] or { NodeType.unknown }
}
